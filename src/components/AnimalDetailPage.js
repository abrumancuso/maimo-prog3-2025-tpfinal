...
export default function AnimalDetailPage() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        setError("");
        const res = await fetch(`${API_URL}/solicitudes-adopcion`, {
          method: "POST",
  ...
  if (!animal) {
    return (
      <main className="min-h-[70vh] flex items-center justify-center">
        <p className="text-sm text-red-500">Animal no encontrado</p>
      </main>
    );
  }
          ...
          <div className="rounded-3xl border border-orange-100 bg-white p-6 shadow-md">
            ...
            <form onSubmit={formik.handleSubmit} className="space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div className="space-y-1">
                  <label className="text-xs font-medium text-zinc-700">
                    En dónde vivís (departamento, casa, etc.)
                  </label>
                  <input
                    type="text"
                    name="tipoVivienda"
